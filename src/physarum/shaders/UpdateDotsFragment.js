...
float sampleDiffuseTexture(vec2 pos, vec3 attract) {
    float val = 0.;
    const float searchArea = 1.;
    const float weight = 1. / 9.;
...
void main()	{
    ...
    float leftVal = sampleDiffuseTexture(leftPos.xy, attract);
    float rightVal = sampleDiffuseTexture(rightPos.xy, attract);
    float midVal = sampleDiffuseTexture(midPos.xy, attract);
    ...
    if (dist< mouseRad) { 
        vec2 dir = seg / max(dist, 0.0001);
        newPosition.xy += isMouseInside * 3. *  dir * (50. + mouseRad -  dist) / (50. + mouseRad /  5.); 
    } 
